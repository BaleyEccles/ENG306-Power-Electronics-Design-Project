...
\date{\today}
\title{ENG306 - Power Electronics - Converter Design Report}
\hypersetup{
 pdfauthor={Baley Eccles - 652137 and Tyler Robards - 651790},
 pdftitle={ENG306 - Power Electronics - Converter Design Report},
 pdfkeywords={},
 pdfsubject={},
...
\label{sec:orgb1834ef}
This report outlines the design and performance analysis of a power converter, it consists of a rectifier stage feeding into a buck converter stage. The design of this converter will require analysis into its efficiency, harmonics and regulation capabilities.
...
\begin{figure}[H]
\centering
\includegraphics[angle=90,width=10cm]{./IMG20251016100928.jpg}
\caption{Designed Converter Running An Arduino Uno \label{fig:BOARD}}
\end{figure}
\section{Design}
\label{sec:org2374347}

\subsection{Specifications}
\label{sec:orgc4516cb}
\begin{table}[H]
...
Voltage Ripple & 0.7\%\\
\hline
DC output & 12\textpm{}0.03\\
\hline
\end{tabular}
\end{table}
\subsection{Calculations And Component Selection}
\label{sec:org2ffcc7b}
\subsubsection{Rectifier}
\label{sec:org648f88c}
...
\section{Modelling And Simulation Analysis}
\label{sec:org2a9af12}
...
\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./Simulation_Schematic.png}
\caption{PLECS Model \label{fig:PLECS_SCHEMATIC}}
\end{figure}

The best case can be seen in Figure \ref{fig:PLECS_BEST_CASE}, it has \(V_{in} = 32V\ RMS\) and a \(24\Omega\) load. It can be seen that the voltage ripple is approximately \(0.5V\), this is more than the acceptable voltage ripple and is due to the PID being tuned poorly. The code that produced this plot can be seen in Appendix B.

\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./ENG306_Design_Best_Case_Sim.png}
\caption{PLECS Best Case Output Voltage \label{fig:PLECS_BEST_CASE}}
\end{figure}

The worst case can be seen in Figure \ref{fig:PLECS_WORST_CASE}, it has \(V_{in} = 20V\ RMS\) and a \(2.4\Omega\) load. It can be seen that the voltage ripple is approximately \(7V\), once again this is more than the acceptable voltage ripple and is due to the PID. The code that produced this plot can be seen in Appendix B.

\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./ENG306_Design_Worst_Case_Sim.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./ENG306_Design_Rectifier_Output_Sim.png}
...
\section{Converter Performance Analysis}
\label{sec:orgce080b5}
...
\hline
\(V_{in}\) RMS (V) & Load (\(\Omega\)) & Ripple (mV) & Ripple \% & V Max (V) & V Min (V) & V mean (V)\\
\hline
...
From the last row in Table \ref{tab:PREFORMANCE_DATA} it can clearly be seen that the required metrics were not fully met, the plot of the acquired for this can be seen in Figure \ref{fig:2_5_20VRMS}. For this specific case the input voltage was \(20VRMS\) and the load was \(2.5\Omega\), this is the worst possible case for the converter. It preformed this poorly because the LM5117 was supplied with less than \(12V\), this caused the converter to stop working for various parts of the cycles which can be seen in Figure \ref{fig:2_5_20VRMS}. This could have been fixed by improving the rectifier output filter, as this provides the voltage to the LM5117.

\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./ENG306_Design_2.5_Ohm_Load_V_in_20_VRMS.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./ENG306_Design_22_Ohm_Load_V_in_32_VRMS.png}
\caption{\(22\Omega\) Load With \(32VRMS\) Input Voltage Output Voltage Plot \label{fig:22_32VRMS}}
\end{figure}

During testing the converter was tested on a laptop and an Arduino Uno. The converter was able to power the Arduino UNO, it did so with very little effort, this cane be seen in Figure \ref{fig:BOARD}. However, the laptop could not be powered, this was due to the laptop requiring \(20V\), not \(12V\). If a laptop required \(12V\) was available it is believed that the converter would have been able to power it.
\section{Efficiency Analysis}
\label{sec:org9981234}
...
\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./ENG306_Design_Efficency_Plot.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./ENG306_Design_Harmonics.png}
...
In this report, we successfully designed and analysed a power converter capable of providing a regulated DC output of 12V with minimal voltage ripple. Challenges related to output voltage regulation were faced, the converter preformed well under various load conditions, as validated by both PLECS simulations and real-world testing.
...
\newpage
\section{Appendix A - R16 And Rb16 Code}
\label{sec:org3cd07d1}
...
\newpage
\section{Appendix B - PLECS Plots Code}
\label{sec:org4d72195}
...
\end{minted}
\section{Appendix C - Testing Code}
\label{sec:orgcb6aff5}
...
title("22 Ohm Load V_{in} = 32 VRMS")
...
title("2.5 Ohm Load V_{in} = 32 VRMS")
...
title("25 Ohm Load V_{in} = 23 VRMS")
...
title("2.5 Ohm Load V_{in} = 20 VRMS")
...
\newpage
\section{Appendix D - Harmonics Plot}
\label{sec:org243338c}
...
\newpage
\section{Appendix E - Efficiency Plot Code}
\label{sec:org7d8481d}
\begin{minted}[]{octave}
clc;
clear;
close all;
